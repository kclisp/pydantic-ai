...
def load_mcp_toolsets(config_path: str | Path) -> list[AbstractToolset[Any]]:
    ...
    Raises:
        ...
        ValueError: If the configuration file does not match the expected `mcpServers` shape, or if
            an environment variable referenced in the configuration is not defined and no default is
            provided.
    """
    ...
    if not isinstance(expanded_config_data, dict) or 'mcpServers' not in expanded_config_data:
        raise ValueError('MCP config must be a JSON object with an `mcpServers` field.')
    servers = cast('dict[str, dict[str, Any]]', expanded_config_data['mcpServers'])
    ...
    for name, entry in servers.items():
        ...
        if 'url' in entry:
            ...
            headers = cast('dict[str, str] | None', entry.get('headers'))
            toolset = MCPToolset(url, id=name, headers=headers)
        else:
            stdio_entry: dict[str, Any] = {
                'command': cast(str, entry['command']),
                'args': list(cast('Sequence[str]', entry.get('args') or [])),
            }
            env = entry.get('env')
            if env is not None:
                stdio_entry['env'] = cast('dict[str, str]', env)
