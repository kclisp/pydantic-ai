...
`TestMCPToolsetConstruction` covers `MCPToolset`'s construction logic: kwarg conflict detection,
HTTP transport adapter for `http_client=`, sampling shortcut, the cache-invalidating message
handler.
...
class TestResourceTypeMapping:
    ...
    `pydantic_ai.mcp.*`. These tests pin the wire-level field mapping so drifts from the MCP SDK
    schema are caught."""
...
class TestLoadMCPToolsets:
    ...
    async def test_load_mcp_toolsets_env_var_expansion(self, monkeypatch: pytest.MonkeyPatch):
        """Environment variable references in the config are expanded; defaults and missing vars covered."""
        monkeypatch.setenv('MCP_URL', 'http://configured.example/mcp')
        config_text = json.dumps(
            {
                'mcpServers': {
                    'alpha': {
                        'url': '${MCP_URL}',
                        'headers': {
                            'Authorization': '${MCP_TOKEN:-default-token}',
                            'Tags': ['${MCP_URL}'],
                        },
                    },
                }
        ...
        # Missing var without a default raises a clear `ValueError`.
        config_text = json.dumps({'mcpServers': {'alpha': {'url': '${MCP_MISSING}'}}})
        with TemporaryDirectory() as tmp:
            config_path = Path(tmp) / 'mcp.json'
            config_path.write_text(config_text)
    ...
    async def test_load_mcp_toolsets_invalid_shape_raises(self):
        with TemporaryDirectory() as tmp:
            config_path = Path(tmp) / 'mcp.json'
    ...
    async def test_load_mcp_toolsets_passes_through_non_string_values(self):
        """JSON `null` and other non-str/dict/list values are passed through env-var expansion unchanged."""
        # Custom optional fields with null/numeric values exercise the fallthrough branch in env-var expansion.
        config_text = json.dumps(
            {
                'mcpServers': {
                    'alpha': {
