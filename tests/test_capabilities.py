...
class TestWebFetchCapability:
    ...
    def test_webfetch_local_string_strategy(self, allow_model_requests: None):
        """WebFetch(local='web_fetch') with non-supporting model → markdownify fallback used."""
        ...
        model = FunctionModel(model_fn, profile=ModelProfile(supported_builtin_tools=frozenset()))
        agent = Agent(model, capabilities=[WebFetch(local='web_fetch')])
        with patch(
    ...
    def test_webfetch_builtin_false_with_local_string(self):
        """WebFetch(builtin=False, local='web_fetch') → only local, no builtin registered."""
        cap = WebFetch(builtin=False, local='web_fetch')
        assert cap.get_builtin_tools() == []
        toolset = cap.get_toolset()
        assert toolset is not None
    ...
    def test_webfetch_domains_forwarded_to_local(self, allow_model_requests: None):
        """WebFetch(allowed_domains=..., local='web_fetch') with non-supporting model → falls back to local with domain filtering."""
        from unittest.mock import AsyncMock, patch
        ...
        model = FunctionModel(model_fn, profile=ModelProfile(supported_builtin_tools=frozenset()))
        agent = Agent(model, capabilities=[WebFetch(allowed_domains=['example.com'], local='web_fetch')])
        with patch(
            'pydantic_ai.common_tools.web_fetch.safe_download', new_callable=AsyncMock, return_value=mock_response
        ):
    ...
    def test_webfetch_builtin_false_with_max_uses_raises(self):
        """WebFetch(builtin=False, local='web_fetch', max_uses=...) → UserError at construction."""
        with pytest.raises(UserError, match='constraint fields require the builtin tool'):
            WebFetch(builtin=False, local='web_fetch', max_uses=5)
...
def test_web_fetch_unknown_strategy_with_extras_missing(monkeypatch: pytest.MonkeyPatch):
    """WebFetch(local='web_fetch') raises with install hint when [web-fetch] extra is missing."""
    import builtins
