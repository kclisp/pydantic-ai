...
Pydantic AI provides [`MCPToolset`][pydantic_ai.mcp.MCPToolset] for connecting agents to local and remote MCP servers. See the [MCP overview](./mcp/overview.md) for details.
...
* [`mcp-run-python`](https://github.com/pydantic/mcp-run-python) - MCP server by the Pydantic team that runs Python code in a sandboxed environment.
